...
public static class StreamCStringExtensions
{
	...
	/// Asynchronously writes the sequence of bytes represented by the <paramref name="cstr"/> to the current
	/// <paramref name="stream"/>,
	/// and advances the current position within the stream by the number of bytes written, including a null-termination
	...
	/// </param>
	/// <returns>A task that represents the asynchronous write operation.</returns>
	public static Task WriteAsync(this Stream stream, CString cstr, Boolean writeNullTermination,
		CancellationToken cancellationToken = default)
		=> cstr.WriteAsync(stream, writeNullTermination, cancellationToken);
	/// <summary>
	/// Asynchronously writes a specific number of bytes from the <paramref name="cstr"/> to the current
	/// <paramref name="stream"/>,
	/// starting at the specified index, and advances the current position within the stream by the number of bytes written.
	/// </summary>
	/// <param name="stream">The current <see cref="Stream"/> instance to which bytes will be written.</param>
