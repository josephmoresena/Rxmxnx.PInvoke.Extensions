...
internal unsafe sealed class ReadOnlyFixedReference<T> : ReadOnlyFixedMemory, IReadOnlyFixedReference<T>, IEquatable<ReadOnlyFixedReference<T>>
    where T : unmanaged
{
    /// <inheritdoc/>
    ...
    {
        IReadOnlyFixedReference<TDestination> result = this.GetTransformation<TDestination>(out ReadOnlyFixedOffset fixedOffset);
