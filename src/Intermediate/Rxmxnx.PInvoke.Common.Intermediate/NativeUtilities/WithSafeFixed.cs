...
public static partial class NativeUtilities
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe void WithSafeFixed<T>(ref T value, FixedReferenceAction<T> action)
        where T : unmanaged
    {
        ArgumentNullException.ThrowIfNull(action);
        fixed (void* ptr = &value)
    ...
    /// <returns>The result of <paramref name="func"/> execution.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe TResult WithSafeFixed<T, TArg, TResult>(ref T value, TArg arg, FixedReferenceFunc<T, TArg, TResult> func)
        where T : unmanaged
    {
        ArgumentNullException.ThrowIfNull(func);
        fixed (void* ptr = &value)
