...
public abstract partial class ValueRegion<T> where T : unmanaged
{
    ...
    /// Thrown when <paramref name="startIndex"/> is less than zero or greater than or equal to the length of the memory region.
    /// </exception>
    ...
    /// or when the specified <paramref name="length"/> is greater than the remaining length from the <paramref name="startIndex"/>.
    /// </exception>
    ...
    /// </summary>
    /// <param name="region">The <see cref="ValueRegion{T}"/> to convert.</param>
    /// <returns>An array representation of the memory region. Returns <see langword="null"/> if the region is not contained in an array.</returns>
