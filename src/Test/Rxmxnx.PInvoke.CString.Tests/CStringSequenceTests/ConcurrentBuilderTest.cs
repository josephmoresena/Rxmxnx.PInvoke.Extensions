...
public sealed class ConcurrentBuilderTest
{
	...
	public void EmptyTest()
	{
		...
		PInvokeAssert.Equal(
			builder,
			builder.ConcurrentAppend(ReadOnlySpan<Byte>.Empty).ConcurrentAppend(default(CString?))
			       .ConcurrentAppend(ReadOnlySpan<Char>.Empty).ConcurrentAppend(default(String?))
			       .ConcurrentAppend(ReadOnlySequence<Byte>.Empty).ConcurrentAppend(CString.Zero)
			       .ConcurrentAppend(CString.Empty).ConcurrentAppend(String.Empty));
		build = builder.Build();
		...
		PInvokeAssert.Equal(
			builder,
			builder.ConcurrentAppendEscaped(ReadOnlySequence<Byte>.Empty)
			       .ConcurrentAppendEscaped(ReadOnlySpan<Byte>.Empty));
		build = builder.Build();
	...
	public void AppendCharSpanTest()
	{
		...
		foreach (String value in TestSet.Utf16Text)
			builder.ConcurrentAppend(value.AsSpan());
	...
	public void AppendSequenceTest()
	{
		...
		foreach (Byte[] value in TestSet.Utf8Bytes)
			builder.ConcurrentAppend(new ReadOnlySequence<Byte>(value));
		PInvokeAssert.Equal(seqRef, builder.Build());
...
#endif
			builder.ConcurrentAppendEscaped(new ReadOnlySequence<Byte>(encoded));
		}
		PInvokeAssert.Equal(seqRef, builder.Build());
	}
	[Theory]
