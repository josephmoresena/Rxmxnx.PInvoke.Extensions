...
public sealed class UnicodeConcurrentTests : CStringBuilderTestsBase
{
	[Theory]
	[InlineData(null)]
	[InlineData(8)]
	...
	public void AppendTest(Int32? length)
	{
		...
		foreach (String? newString in indices.Select(i => TestSet.GetString(i, true)))
		{
			...
			PInvokeAssert.True(Object.ReferenceEquals(cstrBuild, cstrBuild.ConcurrentAppend(newString)));
		}
	...
	public void AppendArrayTest(Int32? length)
	{
		...
		foreach (String? newString in indices.Select(i => TestSet.GetString(i, true)))
		{
			...
			PInvokeAssert.True(Object.ReferenceEquals(cstrBuild, cstrBuild.ConcurrentAppend(chars)));
		}
		PInvokeAssert.Equal(strBuild.ToString(), cstrBuild.ToString());
	}
	[Theory]
	...
	public void AppendLineTest(Int32? length)
	{
		...
		foreach (String? newString in indices.Select(i => TestSet.GetString(i, true)))
		{
			...
			PInvokeAssert.True(Object.ReferenceEquals(cstrBuild, cstrBuild.ConcurrentAppendLine(newString)));
		}
		PInvokeAssert.Equal(strBuild.ToString(), cstrBuild.ToString());
	}
	[Theory]
	...
	public void AppendLineArrayTest(Int32? length)
	{
		...
		foreach (String? newString in indices.Select(i => TestSet.GetString(i, true)))
		{
			strBuild.AppendLine(newString);
			PInvokeAssert.True(
				Object.ReferenceEquals(cstrBuild, cstrBuild.ConcurrentAppendLine(newString?.ToCharArray())));
		}
		PInvokeAssert.Equal(strBuild.ToString(), cstrBuild.ToString());
	}
	[Theory]
	...
	public void InsertTest(Int32? length)
	{
		...
		foreach (String? newString in indices.Select(i => TestSet.GetString(i, true)))
		{
			...
			PInvokeAssert.True(Object.ReferenceEquals(cstrBuild, cstrBuild.ConcurrentInsert(utf8Index, newString)));
		}
		PInvokeAssert.Equal(strBuild.ToString(), cstrBuild.ToString());
