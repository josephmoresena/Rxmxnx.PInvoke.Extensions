		...
		try
		{
			...
			else
			{
				ReadOnlySpan<Byte> bytes1 =
					MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref Unsafe.AsRef(refValue), 1));
				ReadOnlySpan<Byte> bytes2 =
					MemoryMarshal.AsBytes(MemoryMarshal.CreateReadOnlySpan(ref Unsafe.AsRef(refValue2), 1));
