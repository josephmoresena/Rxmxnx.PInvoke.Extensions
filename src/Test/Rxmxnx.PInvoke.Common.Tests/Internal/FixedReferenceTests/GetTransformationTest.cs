...
public sealed class GetTransformationTest : FixedReferenceTestsBase
{
    ...
    private static void Test<T>(FixedReference<T> fref, IntPtr ptr) where T : unmanaged
    {
        ...
        Test<T, Double>(fref);
        Test<T, Decimal>(fref);
        Test<T, DateTime>(fref);
        Test<T, TimeOnly>(fref);
        Test<T, TimeSpan>(fref);

        fref.Unload();
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Boolean>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Byte>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Int16>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Char>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Int32>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Int64>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Int128>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Single>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Half>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Double>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, Decimal>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, DateTime>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, TimeOnly>(fref, true)).Message);
        Assert.Equal(InvalidError, Assert.Throws<InvalidOperationException>(() => Test<T, TimeSpan>(fref, true)).Message);
    }
        ...
        where T2 : unmanaged
    {
        ...
        ReferenceTest(fref, offset, result);
    ...
    private static unsafe void ReferenceTest<T, T2>(FixedReference<T> fref, FixedOffset offset, FixedReference<T2> result)
        where T : unmanaged
        where T2 : unmanaged
    {
    ...
    private static unsafe void ReferenceTest<T, T2>(ReadOnlyFixedReference<T> fref, ReadOnlyFixedOffset offset, ReadOnlyFixedReference<T2> result)
        where T : unmanaged
        where T2 : unmanaged
    {
        HashCode hashResidual = new();
