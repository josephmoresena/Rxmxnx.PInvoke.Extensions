...
public abstract class FixedContextTestsBase : FixedMemoryTestsBase
{
	...
	private protected static unsafe void WithFixed<T>(T[] values, Action<FixedContext<T>, T[]> actionTest)
	{
	...
	private protected static unsafe void WithFixed<T>(T[] values, Action<ReadOnlyFixedContext<T>, T[]> actionTest)
	{
		ReadOnlySpan<T> span = values.AsSpan();
		fixed (void* ptr = &MemoryMarshal.GetReference(span))
			actionTest(new(ptr, span.Length), values);
	...
	/// <param name="actionTest">Action test to <see cref="FixedContext{T}"/> instance to be used.</param>
	private protected static unsafe void WithFixed<T, TObj>(ReadOnlySpan<T> span, TObj obj,
