...
public sealed class EqualsTest : FixedContextTestsBase
{
	...
	private static unsafe void Test<T>(FixedContext<T> ctx, T[] values) where T : unmanaged
	{
		...
		{
			EqualsTest.Test(ctx, values.Length);
			EqualsTest.FalseTest(ctx, values, ptrValue);
		}
	}
	...
	private static void Test<T>(FixedContext<T> ctx, Int32 length) where T : unmanaged
	{
	...
	private static void Test<T>(ReadOnlyFixedContext<T> ctx, Int32 length) where T : unmanaged
	{
		EqualsTest.TransformationTest<T, Boolean>(ctx, length);
		EqualsTest.TransformationTest<T, Byte>(ctx, length);
		EqualsTest.TransformationTest<T, Int16>(ctx, length);
