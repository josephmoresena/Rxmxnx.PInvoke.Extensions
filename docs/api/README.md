...
- **Language floor.** Public constraints require **C# 7.3**. Prefer **C# 11** (UTF-8 literals). On **.NET 9+**, use **C# 13**. Visual Basic gets a minimal helper surface only.
...
- **.NET 9+.** `params ReadOnlySpan<T>` instead of arrays; `allows ref struct` on pointers, wrappers, and many callbacks; **C# 13** for consumers.
